...
\begin{figure}[p]
  \centering
  \rotatebox{-90}{\includegraphics[width=\textheight, height=\textwidth, keepaspectratio]{images/schematic.pdf}}
...
The STLINK-V3PWR probe as we already had it available~\cite{ref4}.
...
(Section~\ref{subsec:rj45}), and the STLINK debug probe. If software reset over SWD fails, the probe can still assert NRST to force a hardware reset~\cite{ref13}.
...
Per the AN2867 oscillator load model~\cite{ref24}, the two external capacitors
$C_{L1}$ and $C_{L2}$ together with PCB stray capacitance~$C_S$ must present the
crystal's specified load~$C_L$:
%
\begin{equation}
  C_L = \frac{C_{L1} \cdot C_{L2}}{C_{L1} + C_{L2}} + C_S
\end{equation}
%
Placing identical capacitors on both sides ($C_{L1} = C_{L2} = C$) simplifies this to
$C = 2(C_L - C_S)$. With $C_L = \SI{10}{\pico\farad}$ and
$C_S = \SI{5}{\pico\farad}$ (AN2867 typical range: 2--\SI{7}{\pico\farad}):
%
\begin{equation}
  C = 2\,(10 - 5) = \SI{10}{\pico\farad}
\end{equation}
%
Two \SI{10}{\pico\farad} C0G capacitors are therefore placed to GND, one on each side
of the crystal. C0G (Class~1) dielectric is used because its capacitance is essentially
independent of temperature and applied voltage --- typically within $\pm30$\,ppm/°C ---
which is important for a stable oscillator load~\cite{ref19,ref20}.
...
SPI was chosen over SDMMC for three reasons: the application does not require SDMMC's
higher throughput; SPI was already in use for the RFID reader, reducing the number of
active interfaces and occupied pins; and while this STM32 series supports SDMMC, it is
only available on larger packages~\cite{ref1}.
...
Kingbright APHBM2012 series the APHBM2012LSURKZGKC. Each LED has independent red and
green elements with common cathodes to GND, so each colour is driven through its own
current-limiting resistor from a GPIO.

\begin{table}[htbp]
  \centering
  \caption{Indicator LED assignments.}
  \label{tab:leds}
  \begin{tabular}{p{0.08\textwidth} p{0.18\textwidth} p{0.30\textwidth} p{0.30\textwidth}}
    \toprule
    Ref  & Nominal current         & Green signal     & Red signal       \\
    \midrule
    LED1 & \SI{2}{\milli\ampere}   & LED\_G1 (PB4)    & LED\_R1 (PA15)   \\
    LED2 & \SI{2}{\milli\ampere}   & LED\_G2 (PB6)    & LED\_R2 (PB5)    \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[p]
  \centering
  \rotatebox{-90}{\includegraphics[width=\textheight, height=\textwidth, keepaspectratio]{images/pcb_f.pdf}}
...
\begin{figure}[p]
  \centering
  \rotatebox{-90}{\includegraphics[width=\textheight, height=\textwidth, keepaspectratio]{images/pcb_b.pdf}}
